...
Um pacote é constituido de funções e objetos do `R` que facilitam rotinas, ou seja, não precisamos repetir um conjunto de códigos, geralmente extenso, em atividades rotineiras.
...
Neste texto, sempre que estiver escrito "`R` Base", estarei referindo-me ao conjutos de pacotes citados acima.
...
Este, destaca-se por ser um conjunto de outros pacotes amplamente usados para análise de dados, fornecendo uma uniformidade na escrita de comandos, bem como uma comunidade ativa em sua implementação.

Como foi dito em nosso Plano de Curso, usaremos o `R` na perspectiva do *tidyverse*.
Então, não será nosso objetivo um aprofundamento na linguagem `R` (que é algo muito maior e demandaria muito tempo).
Apenas falaremos, brevemente, sobre aspectos essenciais, dessa linguagem, que utilizaremos nos pacotes do *tidyverse*.
...
No texto que se segue, haverá caixas de código onde você pode digitar o que se pede.
...
dois números reais, dentro do domínio dessas operações.
...
Mas, vamos começar com coisas pequenas nesse tutorial, ok?
...
Para você saber o tipo de dado no `R`, pode usar o comando `class()`.

Por exemplo, se atribuímos à variável *x* o número $3$, podemos dizer que "*x* é do tipo numérico". Rode o código abaixo e veja.
...
Isso se repetiria caso atribuíssemos à mesma variável, os valores $\sqrt2$, ou $3/4$, ou $0.45$, ou $-1.43$, etc.
...
Para isso, basta colocar um "L" (maiúsculo) do lado do número que deseja direfenciar como inteiro.
Essa diferenciação acontece por conta do processamento interno do `R`.
Ele processa mais rapidamente números inteiros.
Então, se você for trabalhar com dados grandes que possuam números inteiros é bom diferenciar.
Mas, esse não será o caso de nosso curso.
Mesmo assim, se fizermos `x <- 3L`, a classe será *integer*.
...
Falaremos um pouco sobre algumas relações lógicas importantes para o curso, mas, por enquanto, relacione sempre como se fosse uma pergunta ... 
...
Uma parte do *script* é exibido abaixo
...
- **Espaços entre as variáveis**. Não deixe espaços entre as variáveis. Não é obrigatório, mas é uma boa prática de programação. Assim, não escreva `dia 01`, prefira `dia01`, embora essa última não seja a mais adequada, pois poderíamos separar a "frase" do "número" com um *underline*, por exemplo: `dia_01`;
...
Feito essas considerações, alguns autores usam, basicamente, três formas de nomear uma variável:
   ...
   (iii) Separando por *ponto*. Por fim, você pode usar a mesma ideia do item (ii), mas separando por "ponto". Assim: `media.laboratorio`. Esta última, o `R` usa em suas funções dos pacotes básicos.
...
Eles retornarão alguns dos seguintes valores lógicos:

- `TRUE`. Valor lógico *Verdadeiro*;
- `FALSE`. Valor lógico *Falso*.

Ou relacionará variáveis num *Data Frame* (falaremos sobre isso em outra oportunidade).

Rode os seguintes códigos e veja os resultados
...
Algumas dessas estruturas são listadas abaixo:
...
- **Listas.** Estrutura mais gerais de armazenamento. Em uma *lista*__* pode conter um vetor, uma matriz, um data frame ou, até mesmo, outras listas! Usamos o comando `list(var1 = ..., var2 = ..., ...)`
...
Bom ... usaríamos a função `sum()`, do `R`.
...
Poderíamos ter feito: `m <- sum(medias)/10`, para descobrirmos esse valor; mas, no `R`, existe uma função específica para isso: `mean()`.
...
Desses 20 integrantes, 2 são sócios e ganham, cada um, R$ 120.000,00.
Os outros 18 funcionários são empregados dos dois citados e ganham, cada um R$ 1.200,00.

A média salarial dessa empresa seria de $\text{R}\$\;13080,00$.
Esse é um bom número para representar esses dados?
Nossa intuição parece dizer que não, concorda?
...
No caso de nosso exemplo, primeiro deveríamos ordenar os salários (o que é fácil, visto que são 18 iguais a 1.200 e outros 2 iguais a 120.000):
...
Como representaríamos esse conjunto de dados para calcularmos a mediana? 
Será que precisaríamos escrever, pelo menos, 18 itens iguais a 1200?
...
- `a:b`. Produz uma sequência de números de $a$ até $b$, sendo a diferença entre eles de uma unidade. Assim, `1:20` produz a sequência $1, 2, 3, \ldots, 19, 20$.
...
Note que o valor de $R\$\, 1.200,00$ é mais representativo para esse conjunto de dados do que a média aritmética de $\text{R}\$\;13.080,00$.

## Matrizes

Para estruturarmos os dados no formato de uma matriz, usamos o comando `matrix( x, nrow = m, ncol = n)`.

Assim, suponha que você tenha uma sequência de 16 elementos os quais quer estruturá-los numa matriz de ordem 4, ou seja, 4 linhas e 4 colunas.
...
Caso queira que sejam dispostos por linha, acrescente o parâmetro: `byrow = TRUE`.
...
As listas são estruturações mais gerais do `R`, pois pode conter outras estruturas já vistas, bem como outras listas!!
...
Vamos chamar essas medições de `medidas_chuva`. Criamos a lista assim:
...
Ou seja, sendo as observações dadas por `obs <- c(0.4, 1.6, 1.3, 1.4, 0.6, 1.3, 0.7, 0.3, 1.3, 1.7, 0.9, 0.7, 1.6, 1.6, -0.5, 1.2, 1.1, 1.3, 1.0, 1.9, 0.9, 1.2, 1.5, 0.9, 0.9, 1.7, 0.8, 0.5, -0.2, 1.3, 1.0, 1.1, 0.7, 1.6)`, a matriz é dada por  `dados_tabela <- matrix(obs, 17, 2)`.

Bom, toda essa *estória* inventada é para que a mágica acontecesse: vamos criar uma lista que junte todas essas observações!!
...
Vamos nos deparar com diversos *Data Frames*; e, veremos que há uma diferença entre como os humanos enxergam os dados; e, como os computadores leem esses mesmos dados.
Essa diferença é crucial para processamento dos dados no computador.
Portanto, organizaremos *Data Frames* num formato especial: **Tidy**.
...
Vamos denominar esse data frame por `df_medias`. 
Assim, podemos fazer:
...
É possível calcularmos a média aritmética da  varivável `medias`, desse data frame.
...
Não é difícil perceber que um *data frame* pode ser visto como uma matriz, em certo sentido.
Tanto que os comando de seleção de elementos, linhas ou colunas, daquele; podem ser usados num *Data Frame*.
...
Algumas mais simples: quando o número de linhas e colunas coincidem; outras, nem tanto: número de linhas ou colunas diferentes.
...
No semestre "A" existiam 20 estudantes, portanto 20 notas; no semestre "B", 10; e, no semestre "C", 15 notas.
...
Vamos construir um novo data frame que possua como **colunas** essas duas variáveis.

Mas, para isso, precisamos fazer modificações nos data frames originais.
Por exemplo, no primeiro data frame, `semestre_A`, queremos transformá-lo num outro data frame com as variáveis SEMESTRE e NOTA. 
...
Para isso, basta digitar `Orange` e rodar o código. Veja o dataset abaixo:
...
Notamos, também, que as árvores estão agrupadas de alguma forma: "1", "2", etc.
...
Agora, convenhamos ...
Tais gráficos não estão nada agradáveis (esteticamente falando)!
